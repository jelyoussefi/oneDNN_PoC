...
// avacpuilable on the current systems. It is not treated as an error then, but
...
inline float time_elapsed(std::chrono::high_resolution_clock::time_point t_start) {
        std::chrono::high_resolution_clock::time_point t_stop = time_now();
        std::chrono::duration<double, std::milli> diff = std::chrono::duration_cast<std::chrono::duration<double>>(t_stop - t_start);
        return diff.count();
}

#define TIMER_START(suffix) \
    std::chrono::high_resolution_clock::time_point t_start##suffix = time_now();

#define TIMER_STOP(suffix) \
    {                                                                                                               \
        auto diff = time_elapsed(t_start##suffix);                                                        \
        std::cout<<"\t"<<std::string(#suffix)<<" : \t"<<std::fixed<<std::setprecision(1)<<diff<< " ms\n" << std::endl;   \
    }\
